...
<head>
    ...
    <title>Edit Character: {{ $character->name }} - {{ config('app.name') }}</title>
    <x-assets />
...
<body class="bg-slate-950 text-slate-200 font-sans min-h-screen flex flex-col selection:bg-cyan-500 selection:text-white antialiased">
    <x-navbar />
    <main class="flex-grow pt-8 pb-32 sm:pt-12 relative overflow-hidden text-slate-300" x-data="{ 
        ...
        async submitForm(e) { 
            ...
            try { 
                ...
                if (response.ok) { 
                    this.showToast(resData.message || 'Character changes saved successfully!', 'success'); 
                    if(resData.slug) { 
                        const newUrl = `{{ route('characters.edit', ':id') }}`.replace(':id', {{ $character->id }}); 
        ...
        <div class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="mb-10 flex flex-wrap items-end justify-between gap-6 border-b border-white/10 pb-6">
                <div class="space-y-2">
                    <div class="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-widest">
                        <a href="{{ route('characters.index') }}" class="hover:text-cyan-400 transition-colors">Characters</a>
                        <span class="text-slate-700">&bull;</span>
                        <span class="text-slate-400">Editor</span>
                        <span class="text-slate-700">&bull;</span>
                        <span class="text-slate-500 bg-slate-800/50 px-2 py-0.5 rounded-md border border-white/5">#{{ $character->id }}</span>
                    </div>
                    <h1 class="text-3xl sm:text-5xl font-black text-white tracking-tight leading-none">
                        Edit <span class="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 drop-shadow-sm" id="header-name">{{ $character->name }}</span>
                    </h1>
                </div>
                @php
                    // Mengambil tag_name dari relasi apiTags yang source_api-nya adalah danbooru
                    $danbooruTag = $character->apiTags->first(function($tag) {
                        return str_contains(strtolower($tag->source_api), 'danbooru');
                    })?->tag_name;
                @endphp
                <div class="flex items-center gap-3">
                    @if($danbooruTag)
                        <a href="https://danbooru.donmai.us/posts?tags={{ urlencode($danbooruTag) }}+solo" target="_blank" class="px-5 py-2.5 rounded-xl text-sm font-bold text-cyan-400 hover:text-white bg-cyan-500/10 border border-cyan-500/20 hover:border-cyan-500/50 hover:bg-cyan-500/20 transition-all shadow-lg flex items-center gap-2" title="Open Danbooru Search">
                    ...
                    @endif
                    <a href="{{ route('characters.index') }}" class="px-5 py-2.5 rounded-xl text-sm font-bold text-slate-400 hover:text-white bg-slate-900 border border-white/10 hover:border-white/20 hover:bg-slate-800 transition-all shadow-lg">
                        Back to List
                    </a>
                </div>
            </div>
            ...
            <div class="flex flex-col lg:grid lg:grid-cols-12 gap-8 lg:gap-10">
                    ...
                    <div class="sticky top-24 space-y-6">
                        <div class="bg-slate-900/40 border border-white/10 p-2.5 rounded-[2rem] shadow-2xl backdrop-blur-md">
                            <div class="group relative w-full rounded-3xl overflow-hidden cursor-pointer bg-slate-950" onclick="document.getElementById('image').click()">
                                ...
                                <img id="image-preview" src="{{ $character->image['webp'] }}" alt="{{ $character->name }}" class="w-full aspect-square object-cover transition-all duration-700 group-hover:scale-105 group-hover:opacity-60" />
                                <div class="absolute inset-0 z-20 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
                                    ...
                                    <span class="text-white text-xs font-black uppercase tracking-widest drop-shadow-md">Upload Avatar</span>
                                </div>
                                <input type="file" name="image" id="image" form="characterForm" class="hidden" accept="image/*" onchange="previewImage(this)">
                            </div>
                        </div>
                        ...
                        <div class="bg-slate-900/40 border border-white/10 p-6 rounded-3xl shadow-xl backdrop-blur-md space-y-6 relative overflow-hidden">
                            ...
                            <div class="space-y-2">
                                <label for="rating" class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider">Content Rating</label>
                                <div class="relative">
                                    <select id="rating" name="rating" form="characterForm" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white font-medium appearance-none cursor-pointer focus:ring-2 focus:ring-emerald-500/50 outline-none transition-all">
                                        @foreach(['general' => '✅ General', 'sensitive' => '⚠️ Sensitive', 'questionable' => '🔞 R18 (Questionable)', 'explicit' => '🚫 Adult Explicit', 'unknown' => '❓ Uncategorized'] as $key => $label)
                                            <option value="{{ $key }}" {{ old('rating', $character->rating ?? 'unknown') == $key ? 'selected' : '' }}>{{ $label }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="pt-2">
                                <label class="relative flex items-center justify-between cursor-pointer group bg-slate-950/50 border border-slate-800 p-3 rounded-xl hover:border-slate-700 transition-colors">
                                    <span class="text-sm font-bold text-slate-300 group-hover:text-white transition-colors">Debug Mode</span>
                                    <div class="relative">
                                        <input type="hidden" name="debug" value="0" form="characterForm">
                                        <input type="checkbox" name="debug" value="1" form="characterForm" {{ old('debug', $character->debug) ? 'checked' : '' }} class="sr-only peer">
                ...
                <div class="lg:col-span-8 space-y-8">
                    <form id="characterForm" action="{{ route('characters.update', $character->id) }}" method="POST" enctype="multipart/form-data" @submit.prevent="submitForm" class="space-y-8">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="bg-slate-900/60 border border-white/10 p-6 sm:p-8 rounded-[2rem] shadow-xl backdrop-blur-md space-y-6 relative overflow-hidden group">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-2">
                                    <label for="name" class="block text-sm font-bold text-slate-400">Character Name</label>
                                    <div class="relative">
                                        <input type="text" id="name" name="name" value="{{ old('name', $character->name) }}" required class="w-full bg-slate-950 border border-slate-800 rounded-xl pl-4 pr-10 py-3 text-white font-bold text-lg focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all shadow-inner">
                                        <a href="https://www.google.com/search?q={{ urlencode($character->name) }}" target="_blank" title="Search Google" class="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 bg-slate-800 hover:bg-cyan-500/20 text-slate-400 hover:text-cyan-400 rounded-lg transition-all"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg></a>
                                    </div>
                                </div>
                                
                                <div class="space-y-2" x-data="{ clearField() { let el = document.getElementById('slug'); el.value = ''; el.focus(); } }">
                                    <label for="slug" class="block text-sm font-bold text-slate-400">URL Slug</label>
                                    <div class="relative">
                                        <input type="text" id="slug" name="slug" value="{{ old('slug', $character->slug) }}" required class="w-full bg-slate-950 border border-slate-800 rounded-xl pl-4 pr-10 py-3 text-cyan-200 font-mono text-sm focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all shadow-inner">
                            ...
                            <div class="space-y-2">
                                <label for="series" class="block text-sm font-bold text-slate-400">Series Connection</label>
                                <div class="relative z-50">
                                    <select id="series" name="series[]" multiple placeholder="Search series..." class="w-full"></select>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-2" x-data="{ clearField() { let el = document.getElementById('description'); el.value = ''; el.focus(); } }">
                                    ...
                                    <textarea id="description" name="description" rows="4" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-white text-sm leading-relaxed resize-y focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all shadow-inner">{{ old('description', $character->description) }}</textarea>
                                </div>
                                
                                <div class="space-y-2" x-data="{ clearField() { let el = document.getElementById('keywords'); el.value = ''; el.focus(); } }">
                                    <div class="flex justify-between items-end">
                                        <label for="keywords" class="block text-sm font-bold text-slate-400">Keywords / Aliases</label>
                                        <button type="button" @click="clearField()" class="text-[10px] uppercase tracking-wider font-bold text-slate-500 hover:text-red-400 transition-colors">Clear</button>
                                    </div>
                                    <textarea id="keywords" name="keywords" rows="4" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-white text-sm leading-relaxed resize-y focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all shadow-inner">{{ old('keywords', $character->keywords) }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div x-data="{ 
                            ...
                            async generateAndCopyPrompt() { 
                                ...
                                try { 
                                    const response = await fetch('{{ route('seo.character', $character->id) }}', { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } }); 
                            ...
                            applyJson() { 
                                ...
                                try { 
                                    ...
                                    const fields = [ 
                                        { id: 'seo_title', value: data.meta_title || data.seo_title }, 
                                        { id: 'seo_description', value: data.meta_description || data.seo_description }, 
                                        { id: 'seo_keywords', value: data.meta_keywords || data.seo_keywords }, 
                                        { id: 'description', value: data.character_description || data.description } 
                                    ]; 
                                    let appliedCount = 0; 
                                    ...
                                    else alert('Fields already filled or keys do not match. Clear manually to overwrite.'); 
                            ...
                            <div class="space-y-4 bg-slate-950/40 p-5 rounded-2xl border border-slate-800/50">
                                <div x-cloak x-show="generatedPrompt" class="space-y-2">
                                    <div class="flex items-center justify-between">
                                        <span class="text-[10px] font-bold text-purple-400 uppercase tracking-wider">Generated AI Prompt</span>
                                ...
                                <div class="space-y-2">
                                    <div class="flex items-center justify-between">
                                        <label class="text-[10px] font-bold text-emerald-400 uppercase tracking-wider">JSON Data Injector</label>
                                    ...
                                    <div class="flex flex-col sm:flex-row gap-3">
                                        <textarea x-model="jsonInput" rows="1" class="flex-1 w-full bg-slate-900 border border-emerald-500/30 rounded-xl px-4 py-2.5 text-white text-sm font-mono resize-none focus:ring-2 focus:ring-emerald-500/50 outline-none transition-all placeholder-slate-600" placeholder='{"meta_title": "..."}'></textarea>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-2 md:col-span-2">
                                    <label for="seo_title" class="block text-sm font-bold text-slate-400">Optimized Title</label>
                                    <div class="relative">
                                        <input type="text" id="seo_title" name="meta_title" value="{{ old('meta_title', $character->seo_title) }}" class="w-full bg-slate-950 border border-slate-800 rounded-xl pl-4 pr-10 py-3 text-white text-sm focus:ring-2 focus:ring-purple-500/50 outline-none transition-all shadow-inner">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <textarea id="seo_description" name="meta_description" rows="3" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-white text-sm resize-none focus:ring-2 focus:ring-purple-500/50 outline-none transition-all shadow-inner">{{ old('meta_description', $character->seo_description) }}</textarea>
                                </div>
                                
                                <div class="space-y-2">
                                    <div class="flex justify-between items-end">
                                        <label for="seo_keywords" class="block text-sm font-bold text-slate-400">Meta Keywords</label>
                                    ...
                                    <textarea id="seo_keywords" name="meta_keywords" rows="3" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-white text-sm resize-none focus:ring-2 focus:ring-purple-500/50 outline-none transition-all shadow-inner">{{ old('seo_keywords', $character->seo_keywords) }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-4 py-2">
                            ...
                            <span class="text-xs font-bold text-slate-500 uppercase tracking-widest text-center">Secondary Settings & API</span>
                            <div class="h-px bg-slate-800 flex-grow rounded-full"></div>
                        </div>
                        ...
                        <div x-data="{ open: false }" class="bg-slate-900/60 border border-white/10 rounded-3xl shadow-lg backdrop-blur-md relative overflow-hidden group">
                            ...
                            <div x-show="open" x-collapse x-cloak>
                                <div class="px-6 pb-6 pt-2 border-t border-white/5" x-data="relationshipsManager({{ $character->parents->map(fn($p) => ['id' => $p->id, 'text' => $p->name, 'relation_type' => $p->pivot->relation_type])->toJson() }})">
                                    <div class="flex justify-end mb-4">
                                        <button type="button" @click="addRelation()" class="px-3 py-1.5 bg-pink-500/10 text-pink-400 border border-pink-500/20 hover:bg-pink-500 hover:text-white rounded-lg text-xs font-bold transition-all flex items-center gap-1.5">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 4v16m8-8H4"></path></svg> Add Relation
                                        </button>
                                    </div>
                                    <div class="space-y-3">
                                        <template x-for="(rel, index) in relations" :key="rel.key">
                                            <div class="flex flex-col sm:flex-row items-stretch bg-slate-950/80 rounded-xl border border-slate-800 overflow-hidden focus-within:border-pink-500/50 transition-all">
                                                <div class="relative bg-slate-900 border-b sm:border-b-0 sm:border-r border-slate-800 sm:w-40 flex-shrink-0">
                                                    <select :name="`relationships[${index}][relation_type]`" x-model="rel.relation_type" class="w-full h-full bg-transparent text-xs font-black text-pink-400 py-3 px-4 uppercase tracking-wider outline-none cursor-pointer appearance-none relative z-10">
                                                    ...
                                                    <div class="absolute inset-y-0 right-3 flex items-center pointer-events-none text-pink-500/50"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"></path></svg></div>
                                                </div>
                                                <div class="flex-grow p-1.5" x-init="$nextTick(() => { initSelect($el.firstElementChild, rel, index) })"><div x-ignore class="w-full"></div></div>
                                                <button type="button" @click="removeRelation(index)" class="p-3 bg-slate-900 sm:bg-transparent border-t sm:border-t-0 sm:border-l border-slate-800 text-slate-500 hover:text-red-400 hover:bg-red-400/10 transition-colors flex justify-center items-center"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button>
                                            </div>
                                        </template>
                                        <div x-show="relations.length === 0" class="text-center py-6 border border-dashed border-slate-700/50 rounded-xl bg-slate-950/30 text-xs text-slate-500">No relationships established.</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div x-data="{ open: false }" class="bg-slate-900/60 border border-white/10 rounded-3xl shadow-lg backdrop-blur-md relative overflow-hidden group">
                            ...
                            <div x-show="open" x-collapse x-cloak>
                                <div class="px-6 pb-6 pt-2 border-t border-white/5" x-data="apiTagsManager({{ $character->apiTags->toJson() }})">
                                    <div class="flex justify-end mb-4">
                                    ...
                                    <div class="space-y-3">
                                        <template x-for="(tag, index) in tags" :key="index">
                                            <div class="flex flex-col sm:flex-row items-stretch bg-slate-950/80 rounded-xl border border-slate-800 overflow-hidden focus-within:border-indigo-500/50 transition-all">
                                                <input type="text" x-model="tag.source_api" :name="`api_tags[${index}][source_api]`" class="sm:w-32 bg-slate-900 border-b sm:border-b-0 sm:border-r border-slate-800 text-xs font-black text-indigo-400 py-3 px-4 uppercase text-center outline-none" placeholder="SOURCE">
                                                <input type="text" x-model="tag.tag_name" :name="`api_tags[${index}][tag_name]`" class="flex-grow bg-transparent py-3 px-4 text-sm font-mono text-slate-200 outline-none" placeholder="tag_name_here">
                                                <div class="flex items-center border-t sm:border-t-0 sm:border-l border-slate-800 bg-slate-900 sm:bg-transparent">
                                                    <a x-show="tag.source_api && tag.tag_name" :href="buildLink(tag.source_api, tag.tag_name)" target="_blank" class="p-3 text-slate-400 hover:text-cyan-400 transition-colors flex-1 text-center" title="Test Link"><svg class="w-5 h-5 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg></a>
        ...
        <div class="fixed bottom-0 left-0 right-0 z-50 bg-slate-950/80 backdrop-blur-xl border-t border-white/10 py-4 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
            <div class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4">
                <form action="{{ route('characters.delete', $character->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to permanently delete this character? This action cannot be undone.');" class="w-full sm:w-auto">
                    @csrf @method('DELETE')
    ...
    <script>
    const PRELOAD_SERIES = @json($preloadSeries ?? []);
        ...
        Alpine.data('apiTagsManager', (initialTags) => ({
            ...
            addTag() { 
                ...
                if (targetSource === 'danbooru' || targetSource === 'gelbooru') {
                    targetValue = charName.toLowerCase().replace(/\s+/g, '_');
            ...
            buildLink(source, tag) {
                ...
                const t = tag.trim();
                if (s.includes('danbooru')) return `https://danbooru.donmai.us/posts?tags=${t}+solo+rating%3Ageneral`;
                if (s.includes('gelbooru')) return `https://gelbooru.com/index.php?page=post&s=list&tags=${t}+solo+rating%3Ageneral`;
                if (s.includes('yandere'))  return `https://yande.re/post?tags=${t}`;
                if (s.includes('zerochan')) return `https://www.zerochan.net/${t}`;
                return `https://www.google.com/search?q=${t}+site:${s}.com`;
            }
        }));
        Alpine.data('relationshipsManager', (initialData) => ({
            relations: initialData.map(r => ({ ...r, key: Math.random().toString(36).substr(2, 9) })) || [],
            addRelation() {
                this.relations.push({ id: '', text: '', relation_type: 'variant', key: Math.random().toString(36).substr(2, 9) });
            },
            removeRelation(index) {
                this.relations.splice(index, 1);
            },
            initSelect(containerEl, rel, index) {
                const selectEl = document.createElement('select');
                selectEl.name = `relationships[${index}][parent_id]`;
                selectEl.placeholder = "Search character...";
                containerEl.appendChild(selectEl);
                new TomSelect(selectEl, {
                    valueField: 'id', labelField: 'text', searchField: 'text', maxItems: 1, dropdownParent: 'body',
                    options: rel.id ? [{ id: rel.id, text: rel.text }] : [],
                    items: rel.id ? [rel.id] : [],
                    load: (query, callback) => {
                        let fullUrl = `{{ route('characters.list') }}?q=${encodeURIComponent(query)}&exclude={{ $character->id }}`;
                        fetch(fullUrl).then(r => r.json()).then(j => callback(j)).catch(() => callback());
                    },
                    render: {
                        option: (data, escape) => `<div class="px-2 py-1 text-xs text-slate-300 hover:text-white">${escape(data.text)}</div>`,
                        item: (data, escape) => `<div title="${escape(data.text)}" class="text-xs font-bold text-white">${escape(data.text)}</div>`
                    }
                });
            }
        }));
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const apiSelectConfig = (url, options = {}) => ({
            valueField: 'id', labelField: 'text', searchField: 'text', create: false, plugins: ['remove_button'],
            options: options.preloadOptions || [], items: options.preloadItems || [],
            load: (query, callback) => {
                let fullUrl = `${url}?q=${encodeURIComponent(query)}`;
                if (options.excludeId) fullUrl += `&exclude=${options.excludeId}`;
                fetch(fullUrl).then(r => r.json()).then(j => callback(j)).catch(() => callback());
            },
            render: {
                option: (data, escape) => `<div class="px-2 py-1 text-xs">${escape(data.text)}</div>`,
                item: (data, escape) => `<div title="${escape(data.text)}">${escape(data.text)}</div>`
            }
        });
        if(document.getElementById('series')) {
            new TomSelect('#series', apiSelectConfig("{{ route('series.list') }}", { 
                preloadOptions: PRELOAD_SERIES, 
                preloadItems: PRELOAD_SERIES.map(i => i.id) 
            }));
        }
        const nameInput = document.getElementById('name');
        ...
        if(nameInput) {
            nameInput.addEventListener('input', () => {
                ...
                if(headerName) headerName.textContent = val || 'Character';
                const googleLink = document.getElementById('google-search-link');
                if (googleLink) googleLink.href = `https://www.google.com/search?q=${encodeURIComponent(val)}`;
                const currentSlug = slugInput.value;
                const expectedSlug = val.slice(0, -1).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '');
