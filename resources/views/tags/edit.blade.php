...
<head>
    ...
    <title>Edit Tag: {{ $tag->name }} - {{ config('app.name') }}</title>
    ...
    <main class="flex-grow pt-8 pb-32 sm:pt-12 relative overflow-hidden text-slate-300" x-data="{ 
        ...
        async submitForm(e) { 
            ...
            try { 
                ...
                if (response.ok) { 
                    this.showToast(resData.message || 'Tag changes saved successfully!', 'success'); 
                    if(resData.updated_at) { 
                        const infoEl = document.getElementById('info-updated-at'); 
                        if(infoEl) infoEl.innerText = resData.updated_at; 
                    } 
                    if(resData.slug) { 
                        const newUrl = `{{ route('tags.edit', ':id') }}`.replace(':id', {{ $tag->id }}); 
        ...
        <div class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="mb-10 flex flex-wrap items-end justify-between gap-6 border-b border-white/10 pb-6">
                <div class="space-y-2">
                    <div class="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-widest">
                        <a href="{{ route('tags.index') }}" class="hover:text-cyan-400 transition-colors">Tags</a>
                        <span class="text-slate-700">&bull;</span>
                        <span class="text-slate-400">Editor</span>
                        <span class="text-slate-700">&bull;</span>
                        <span class="text-slate-500 bg-slate-800/50 px-2 py-0.5 rounded-md border border-white/5">#{{ $tag->id }}</span>
                    </div>
                    <h1 class="text-3xl sm:text-5xl font-black text-white tracking-tight leading-none">
                        Edit <span class="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 drop-shadow-sm" id="header-name">{{ $tag->name }}</span>
                    </h1>
                </div>
                @php
                    // Mengambil tag_name dari relasi apiTags yang source_api-nya adalah danbooru
                    $danbooruTag = $tag->apiTags->first(function($apiTag) {
                        return str_contains(strtolower($apiTag->source_api), 'danbooru');
                    })?->tag_name;
                @endphp
                <div class="flex items-center gap-3">
                    @if($danbooruTag)
                        <a href="https://danbooru.donmai.us/posts?tags={{ urlencode($danbooruTag) }}+rating%3Ageneral" target="_blank" class="px-5 py-2.5 rounded-xl text-sm font-bold text-cyan-400 hover:text-white bg-cyan-500/10 border border-cyan-500/20 hover:border-cyan-500/50 hover:bg-cyan-500/20 transition-all shadow-lg flex items-center gap-2" title="Open Danbooru Search">
                    ...
                    @endif
                    <a href="{{ route('tags.index') }}" class="px-5 py-2.5 rounded-xl text-sm font-bold text-slate-400 hover:text-white bg-slate-900 border border-white/10 hover:border-white/20 hover:bg-slate-800 transition-all shadow-lg">
                        Back to List
                    </a>
                </div>
            </div>
            ...
            <div class="flex flex-col lg:grid lg:grid-cols-12 gap-8 lg:gap-10">
                    ...
                    <div class="sticky top-24 space-y-6">
                        ...
                        <div>
                            <div class="bg-slate-900/40 border border-white/10 p-2.5 rounded-[2rem] shadow-2xl backdrop-blur-md">
                                <div class="group relative w-full rounded-3xl overflow-hidden cursor-pointer bg-slate-950" onclick="document.getElementById('image').click()">
                                    ...
                                    <img id="image-preview" src="{{ $tag->image['webp'] }}" alt="{{ $tag->name }}" class="w-full aspect-square object-cover transition-all duration-700 group-hover:scale-105 group-hover:opacity-60" />
                                    ...
                                    <div class="absolute inset-0 z-20 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
                                        ...
                                        <span class="text-white text-xs font-black uppercase tracking-widest drop-shadow-md">Replace Icon</span>
                                    </div>
                                    <input type="file" name="image" id="image" form="tagForm" class="hidden" accept="image/*" onchange="previewImage(this)">
                                </div>
                            </div>
                        ...
                        <div class="bg-slate-900/40 border border-white/10 p-6 rounded-3xl shadow-xl backdrop-blur-md space-y-6 relative overflow-hidden">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative">
                                    <select id="rating" name="rating" form="tagForm" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white font-medium appearance-none cursor-pointer focus:ring-2 focus:ring-emerald-500/50 outline-none transition-all">
                                        @foreach(['general' => '✅ General', 'sensitive' => '⚠️ Sensitive', 'questionable' => '🔞 R18 (Questionable)', 'explicit' => '🚫 Adult Explicit', 'unknown' => '❓ Uncategorized'] as $key => $label)
                                            <option value="{{ $key }}" {{ old('rating', $tag->rating ?? 'unknown') == $key ? 'selected' : '' }}>{{ $label }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="pt-2">
                                <label class="relative flex items-center justify-between cursor-pointer group bg-slate-950/50 border border-slate-800 p-3 rounded-xl hover:border-slate-700 transition-colors">
                                    ...
                                    <div class="relative">
                                        <input type="hidden" name="debug" value="0" form="tagForm">
                                        <input type="checkbox" name="debug" value="1" form="tagForm" {{ old('debug', $tag->debug) ? 'checked' : '' }} class="sr-only peer">
                ...
                <div class="lg:col-span-8 space-y-8">
                    <form id="tagForm" action="{{ route('tags.update', $tag->id) }}" method="POST" enctype="multipart/form-data" @submit.prevent="submitForm" class="space-y-8">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="bg-slate-900/60 border border-white/10 p-6 sm:p-8 rounded-[2rem] shadow-xl backdrop-blur-md space-y-6 relative overflow-hidden group">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-2">
                                    <label for="name" class="block text-sm font-bold text-slate-400">Tag Name</label>
                                    <div class="relative">
                                        <input type="text" id="name" name="name" value="{{ old('name', $tag->name) }}" required class="w-full bg-slate-950 border border-slate-800 rounded-xl pl-4 pr-10 py-3 text-white font-bold text-lg focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all shadow-inner">
                                        <a href="https://www.google.com/search?q={{ urlencode($tag->name) }}" target="_blank" title="Search Google" class="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 bg-slate-800 hover:bg-cyan-500/20 text-slate-400 hover:text-cyan-400 rounded-lg transition-all"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg></a>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-2" x-data="{ clearField() { let el = document.getElementById('slug'); el.value = ''; el.focus(); } }">
                                    ...
                                    <div class="relative">
                                        <input type="text" id="slug" name="slug" value="{{ old('slug', $tag->slug) }}" required class="w-full bg-slate-950 border border-slate-800 rounded-xl pl-4 pr-10 py-3 text-cyan-200 font-mono text-sm focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all shadow-inner">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-2" x-data="{ clearField() { let el = document.getElementById('keywords'); el.value = ''; el.focus(); } }">
                                    ...
                                    <textarea id="keywords" name="keywords" rows="4" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-white text-sm leading-relaxed resize-y focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all shadow-inner">{{ old('keywords', $tag->keywords) }}</textarea>
                                </div>
                                ...
                                <div class="space-y-2" x-data="{ clearField() { let el = document.getElementById('description'); el.value = ''; el.focus(); } }">
                                    ...
                                    <textarea id="description" name="description" rows="4" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-white text-sm leading-relaxed resize-y focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all shadow-inner">{{ old('description', $tag->description) }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div x-data="{ 
                            ...
                            async generateAndCopyPrompt() { 
                                ...
                                try { 
                                    const response = await fetch('{{ route('seo.tags', $tag->id) }}', { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } }); 
                            ...
                            applyJson() { 
                                ...
                                try { 
                                    ...
                                    const fields = [ 
                                        ...
                                        { id: 'description', value: data.tag_description || data.description } 
                                    ]; 
                                    let appliedCount = 0; 
                                    ...
                                    else alert('JSON fields are already filled or keys do not match. Clear the fields manually to replace them.'); 
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-2 md:col-span-2">
                                    ...
                                    <div class="relative">
                                        <input type="text" id="seo_title" name="meta_title" value="{{ old('meta_title', $tag->seo_title) }}" class="w-full bg-slate-950 border border-slate-800 rounded-xl pl-4 pr-10 py-3 text-white text-sm focus:ring-2 focus:ring-purple-500/50 outline-none transition-all shadow-inner">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <textarea id="seo_description" name="meta_description" rows="3" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-white text-sm resize-none focus:ring-2 focus:ring-purple-500/50 outline-none transition-all shadow-inner">{{ old('meta_description', $tag->seo_description) }}</textarea>
                                </div>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <textarea id="seo_keywords" name="meta_keywords" rows="3" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-white text-sm resize-none focus:ring-2 focus:ring-purple-500/50 outline-none transition-all shadow-inner">{{ old('meta_keywords', $tag->seo_keywords) }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-4 py-2">
                            ...
                            <span class="text-xs font-bold text-slate-500 uppercase tracking-widest text-center">Secondary Setting</span>
                            <div class="h-px bg-slate-800 flex-grow rounded-full"></div>
                        </div>
                        ...
                        <div x-data="{ open: false }" class="bg-slate-900/60 border border-white/10 rounded-3xl shadow-lg backdrop-blur-md relative overflow-hidden group">
                            ...
                            <div x-show="open" x-collapse x-cloak>
                                <div class="px-6 pb-6 pt-2 border-t border-white/5" x-data="apiTagsManager({{ $tag->apiTags->toJson() }})">
        ...
        <div class="fixed bottom-0 left-0 right-0 z-50 bg-slate-950/80 backdrop-blur-xl border-t border-white/10 py-4 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
            <div class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4">
                <form action="{{ route('tags.delete', $tag->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to permanently delete this tag? Characters and wallpapers associated with it will lose their relation and this action cannot be undone.');" class="w-full sm:w-auto">
                    @csrf @method('DELETE')
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if(nameInput) {
            nameInput.addEventListener('input', () => {
                ...
                if(headerName) headerName.textContent = val || 'Tag';
