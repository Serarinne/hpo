...
<head>
    ...
    <title>Tag Management - {{ env('APP_NAME') }}</title>
    <x-assets />
    <style>
...
<body class="bg-slate-950 text-slate-200 font-sans min-h-screen flex flex-col selection:bg-cyan-500 selection:text-white">
            ...
            <div class="mb-8 flex flex-wrap items-end justify-between gap-6 border-b border-white/10 pb-6">
                <div class="space-y-2">
                    ...
                    <h1 class="text-3xl sm:text-5xl font-black text-white tracking-tight leading-none">
                        Tag <span class="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 drop-shadow-sm">Management</span>
                    </h1>
                    <p class="text-sm text-slate-400 font-medium pt-1">Browse, filter, and manage all your categorization tags.</p>
                </div>
            </div>
                ...
                <div class="relative z-10 flex flex-col gap-5 p-5 sm:p-6">
                    ...
                    <div class="w-full relative z-10">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="search" id="search" value="{{ request('search') }}" placeholder="Search by tag name or description..." class="w-full bg-slate-950 border border-slate-800 rounded-xl pl-10 pr-4 py-3 text-sm text-white placeholder-slate-600 focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all shadow-inner">
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col md:flex-row gap-4 items-end flex-wrap relative z-20">
                        ...
                        <div class="w-full md:w-44 relative">
                            <label class="block text-[10px] font-bold text-slate-400 mb-1.5 uppercase tracking-wider">Wallpaper Status</label>
                            <details class="group custom-dropdown relative">
                                <summary class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all shadow-inner cursor-pointer list-none flex justify-between items-center select-none">
                                    @php $hasWpCount = count(request('has_wallpaper', [])); @endphp
                                    <span class="text-sm truncate mr-2 {{ $hasWpCount > 0 ? 'text-pink-400 font-bold' : 'text-slate-400 font-medium' }}">{{ $hasWpCount > 0 ? $hasWpCount . ' Selected' : 'Any Status' }}</span>
                                    <svg class="w-4 h-4 text-slate-500 group-open:rotate-180 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7" /></svg>
                                </summary>
                                <div class="absolute top-full left-0 z-50 w-full mt-2 bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl p-2 flex flex-col gap-1">
                                    @foreach(['yes' => 'Has Wallpaper', 'no' => 'No Wallpaper'] as $val => $label)
                                        <label class="flex items-center gap-3 px-3 py-2 hover:bg-slate-800/50 rounded-lg cursor-pointer transition-colors">
                                            <input type="checkbox" name="has_wallpaper[]" value="{{ $val }}" @checked(in_array($val, request('has_wallpaper', []))) class="w-4 h-4 rounded border-slate-700 bg-slate-950 text-pink-500 focus:ring-pink-500 focus:ring-offset-slate-900 shadow-inner">
                        ...
                        <div class="w-full md:w-auto flex flex-1 gap-3 md:justify-end">
                            ...
                            @if(request()->anyFilled(['search', 'rating', 'seo', 'workflow', 'has_wallpaper']))
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 mb-12 relative z-10">
                @forelse($tags as $tag)
                    @php
                        ...
                        $currentRating = $ratingStyles[$tag->rating] ?? $ratingStyles['unknown'];
                    @endphp
                    
                    <a href="{{ route('tags.edit', ['id' => $tag->id]) }}" class="group block relative rounded-[1.5rem] overflow-hidden border border-white/5 bg-slate-900/60 shadow-lg hover:shadow-[0_0_20px_rgba(34,211,238,0.15)] hover:border-cyan-500/30 transition-all duration-300 backdrop-blur-sm transform hover:-translate-y-1">
                        ...
                        <div class="aspect-square overflow-hidden bg-slate-950 relative {{ !$hasImage ? 'flex justify-center items-center p-6' : '' }}">
                            @if($hasImage)
                                <picture class="w-full h-full block">
                                    ...
                                    <img src="{{ $tag->image['jpg'] }}" alt="{{ $tag->name }} Anime Wallpaper Category" loading="lazy" width="400" height="400" class="w-full h-full object-cover group-hover:scale-105 group-hover:opacity-80 transition-all duration-700 ease-out" />
                                </picture>
                            @else
                        ...
                        <div class="absolute top-3 left-3 flex flex-col gap-1.5 z-30">
                            <button type="button" x-data="{ isDebug: {{ $tag->debug ? 'true' : 'false' }} }" @click.prevent.stop="isDebug = !isDebug; toggleDebug({{ $tag->id }}, isDebug)" class="pointer-events-auto h-7 px-2 rounded-lg border backdrop-blur-md transition-all flex items-center justify-center shadow-lg font-bold" :class="isDebug ? 'bg-amber-500/20 text-amber-400 border-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.4)]' : 'bg-slate-950/60 text-slate-400 border-white/10 hover:border-white/20 hover:text-white hover:bg-slate-900'" title="Toggle Debug Mode">
                        ...
                        <div class="absolute top-3 right-3 z-30">
                            <button type="button" x-data @click.prevent.stop="toggleRatingDropdown({{ $tag->id }}, $event)" class="h-7 px-2.5 rounded-lg border backdrop-blur-md transition-all flex items-center justify-center shadow-lg font-black text-[10px] tracking-widest {{ $currentRating['class'] }}" data-rating="{{ $tag->rating }}">
                                <span class="rating-text">{{ $currentRating['text'] }}</span>
                            </button>
                        </div>

                        <div class="absolute inset-x-0 bottom-0 p-4">
                            <h3 class="text-white font-bold text-sm leading-snug line-clamp-1 group-hover:text-cyan-400 transition-colors">{{ $tag->name }}</h3>
                            <div class="mt-2 flex items-center gap-2">
                                <span class="bg-white/10 px-2 py-1 rounded-md text-[9px] uppercase font-bold tracking-wider backdrop-blur-sm border border-white/5 text-slate-300">{{ $tag->rating ?? 'Unknown' }}</span>
                            </div>
                        </div>
                    </a>
                @empty
                    <div class="col-span-full w-full flex flex-col items-center justify-center py-20 px-4 text-center bg-slate-900/40 border border-dashed border-slate-700 rounded-[2rem] backdrop-blur-md shadow-inner">
                        <div class="w-20 h-20 mb-5 rounded-full bg-slate-800 flex items-center justify-center">
                            <svg class="w-10 h-10 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14"></path></svg>
                        </div>
                        <h3 class="text-lg font-black text-white mb-2">No Tags Found</h3>
                        <p class="text-sm font-medium text-slate-500 max-w-sm mx-auto">
                            {{ request('search') ? "We couldn't find any tags matching your current filter criteria." : "There are no tags available yet." }}
                        </p>
                        @if(request()->anyFilled(['search', 'rating', 'seo', 'workflow', 'has_wallpaper']))
    ...
    <script>
        ...
        window.toggleDebug = async function(tagId, isDebug) {
            try {
                const urlTemplate = "{{ route('tags.toggle-debug', ['id' => '__ID__']) }}";
                const url = urlTemplate.replace('__ID__', tagId);
        ...
        window.toggleRatingDropdown = async function(tagId, event) {
            ...
            if (selectedRating && selectedRating !== currentRating) {
                try {
                    ...
                    const urlTemplate = "{{ route('tags.update-rating', ['id' => '__ID__']) }}";
                    const url = urlTemplate.replace('__ID__', tagId);
                    const response = await fetch(url, {
                        method: 'PATCH',
                        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' },
