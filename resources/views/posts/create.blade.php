    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function slugify(text) {
                return text.toString().toLowerCase().trim().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9\s-]/g, '').replace(/\s+/g, '-').replace(/-+/g, '-').replace(/^-+|-+$/g, '');
            }
...
<body class="min-h-screen bg-slate-950 text-slate-200 font-sans">
    ...
    <main class="flex-grow pt-8 pb-32 sm:pt-12 relative overflow-hidden text-slate-300" x-data="{ 
            ...
            try { 
                ...
                if (response.ok) { 
                    ...
                    if (response.status === 422 && resData.errors) { 
                        ...
                        this.showToast(resData.message || 'Failed to publish post.', 'error'); 
                    } 
                } 
            } catch (err) { 
        ...
        <div class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="mb-8 flex flex-wrap items-end justify-between gap-6 border-b border-white/10 pb-6">
                <div class="space-y-2">
                    <div class="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-widest">
                        ...
                        <span class="text-slate-700">&bull;</span>
                        <span class="text-slate-500 bg-slate-800/50 px-2 py-0.5 rounded-md border border-white/5">New</span>
                    </div>
                    <h1 class="text-3xl sm:text-5xl font-black text-white tracking-tight leading-none">
                        Add <span class="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 drop-shadow-sm">New Post</span>
                    </h1>
                    <p class="text-sm text-slate-400 font-medium pt-1">Compose your article, manage settings, and publish from one screen.</p>
                </div>
            ...
            <form id="post-form" action="{{ route('posts.store') }}" method="POST" enctype="multipart/form-data" @submit.prevent="submitForm">
                @csrf
                        ...
                        <div class="bg-slate-900/60 border border-white/10 p-6 sm:p-8 rounded-[2rem] shadow-xl backdrop-blur-md space-y-6 relative overflow-hidden group">
                            ...
                            <div class="space-y-6">
                                <div>
                                    ...
                                    <input type="text" name="title" id="title" value="{{ old('title') }}" required placeholder="Enter an engaging title here..." class="w-full bg-slate-950 border border-slate-800 rounded-xl px-5 py-4 text-white font-black text-2xl sm:text-3xl focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all shadow-inner placeholder-slate-700">
                                </div>

                                <div x-data="{ slug: '{{ old('slug') }}', generateSlug() { this.slug = document.getElementById('title').value.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, ''); } }">
                                    ...
                                    <div class="flex flex-col sm:flex-row overflow-hidden rounded-xl border border-slate-800 bg-slate-950 focus-within:ring-2 focus-within:ring-cyan-500/50 transition-all shadow-inner">
                                        ...
                                        <input type="text" name="slug" id="slug" x-model="slug" required class="flex-1 bg-transparent px-4 py-3.5 text-sm text-cyan-300 font-mono focus:outline-none placeholder-slate-700" placeholder="your-post-slug-here">
                                    </div>
                                </div>
                                ...
                                <div class="pt-2">
                                    ...
                                    <textarea name="body" id="body" rows="18" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-5 text-slate-200 text-sm leading-relaxed resize-y focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all shadow-inner placeholder-slate-700" placeholder="Write your article content here...">{{ old('body') }}</textarea>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="bg-slate-900/40 border border-white/10 p-6 rounded-3xl shadow-xl backdrop-blur-md space-y-6 relative overflow-hidden">
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="datetime-local" name="published_at" id="published_at" value="{{ old('published_at') }}" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white focus:ring-2 focus:ring-emerald-500/50 outline-none transition-all shadow-inner [color-scheme:dark]">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="bg-slate-900/40 border border-white/10 p-4 sm:p-5 rounded-[2rem] shadow-xl backdrop-blur-md">
                                ...
                                <label for="featured_image" class="block cursor-pointer">
                                    <div class="rounded-xl border border-dashed border-slate-700 bg-slate-950/70 p-3 transition-colors hover:border-cyan-500">
                                        <div class="mb-3 flex h-44 items-center justify-center overflow-hidden rounded-lg bg-slate-900 relative">
                                            <img id="featured-image-preview" class="hidden h-full w-full object-cover absolute inset-0 z-10" alt="Preview">
                                            <span id="featured-image-placeholder" class="text-sm text-slate-500 z-0">Upload image preview</span>
                                        </div>
                                        <div class="text-sm font-medium text-slate-200">Choose Featured Image</div>
                                        <div id="featured-image-name" class="mt-1 text-xs text-slate-500">No file selected</div>
                                    </div>
                                </label>
                            ...
                            <div class="bg-slate-900/40 border border-white/10 p-6 rounded-3xl shadow-xl backdrop-blur-md space-y-5 relative overflow-hidden" x-data="{ showNew: false }">
                                <div class="flex items-center justify-between border-b border-white/5 pb-3">
                                    <h3 class="text-xs font-black uppercase tracking-widest text-slate-300 flex items-center gap-2">
                                        <span class="w-1.5 h-1.5 rounded-full bg-slate-400"></span> Categorization
                                    </h3>
                                ...
                                <div class="space-y-4 pt-1">
                                    <div>
                                        <label for="category_id" class="block text-[11px] font-bold text-slate-400 mb-1.5 uppercase tracking-wider">Main Category</label>
                                        <div class="relative">
                                            <select id="category_id" name="category_id" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white font-medium appearance-none cursor-pointer focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all shadow-inner">
                                    ...
                                    <div x-show="showNew" x-collapse x-cloak class="space-y-3 bg-slate-950/60 border border-slate-800 p-4 rounded-2xl shadow-inner mt-2">
                                        ...
                                        <button type="button" onclick="typeof createCategory === 'function' ? createCategory() : alert('Category saved!')" class="w-full bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 rounded-xl px-4 py-2.5 text-[11px] font-bold uppercase tracking-wider transition-colors shadow-sm mt-1">
                                            Save Category
                                        </button>
            ...
            <div class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-end gap-4">
                <div class="flex items-center w-full sm:w-auto gap-4">
                    ...
                    <button type="submit" form="post-form" :disabled="isSaving" class="flex-1 sm:flex-none px-8 py-3 rounded-xl font-black text-slate-950 bg-gradient-to-r from-cyan-400 to-blue-500 hover:from-cyan-300 hover:to-blue-400 shadow-[0_0_20px_rgba(34,211,238,0.2)] hover:shadow-[0_0_30px_rgba(34,211,238,0.4)] transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-wait">
                        ...
                        <span x-text="isSaving ? 'Saving...' : 'Publish / Save'"></span>
                    </button>
                </div>
            </div>
        </div>
