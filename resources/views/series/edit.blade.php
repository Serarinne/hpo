...
<head>
    ...
    <title>Edit Series: {{ $series->name }} - {{ config('app.name') }}</title>
    ...
    <style>
        ...
        .ts-control { background-color: rgba(2, 6, 23, 0.6) !important; border: 1px solid rgba(71, 85, 105, 0.4) !important; color: #f8fafc !important; border-radius: 0.75rem !important; padding: 12px 12px !important; box-shadow: none !important; }
        .ts-control.focus { border-color: #06b6d4 !important; box-shadow: 0 0 0 2px rgba(6, 182, 212, 0.2) !important; }
        ...
        .ts-control .item { background-color: rgba(6, 182, 212, 0.15) !important; border: 1px solid rgba(6, 182, 212, 0.3) !important; color: #22d3ee !important; border-radius: 6px !important; padding: 2px 8px !important; }
    </style>
</head>
<body class="bg-slate-950 text-slate-200 font-sans min-h-screen flex flex-col selection:bg-cyan-500 selection:text-white antialiased">
    ...
    <main class="flex-grow pt-8 pb-32 sm:pt-12 relative overflow-hidden text-slate-300" x-data="{ 
        ...
        async submitForm(e) { 
            ...
            try { 
                ...
                if (response.ok) { 
                    this.showToast(resData.message || 'Series changes saved successfully!', 'success'); 
                    if(resData.updated_at) { 
                        const infoEl = document.getElementById('info-updated-at'); 
                        if(infoEl) infoEl.innerText = resData.updated_at; 
                    } 
                    if(resData.slug) { 
                        const newUrl = `{{ route('series.edit', ':id') }}`.replace(':id', {{ $series->id }}); 
        ...
        <div class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="mb-10 flex flex-wrap items-end justify-between gap-6 border-b border-white/10 pb-6">
                <div class="space-y-2">
                    <div class="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-widest">
                        <a href="{{ route('series.index') }}" class="hover:text-cyan-400 transition-colors">Series</a>
                        <span class="text-slate-700">&bull;</span>
                        <span class="text-slate-400">Editor</span>
                        <span class="text-slate-700">&bull;</span>
                        <span class="text-slate-500 bg-slate-800/50 px-2 py-0.5 rounded-md border border-white/5">#{{ $series->id }}</span>
                    </div>
                    <h1 class="text-3xl sm:text-5xl font-black text-white tracking-tight leading-none">
                        Edit <span class="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 drop-shadow-sm" id="header-name">{{ $series->name }}</span>
                    </h1>
                </div>
                <a href="{{ route('series.index') }}" class="px-5 py-2.5 rounded-xl text-sm font-bold text-slate-400 hover:text-white bg-slate-900 border border-white/10 hover:border-white/20 hover:bg-slate-800 transition-all shadow-lg">
                    Back to List
                </a>
            </div>
            ...
            <div class="flex flex-col lg:grid lg:grid-cols-12 gap-8 lg:gap-10">
                    ...
                    <div class="sticky top-24 space-y-6">
                        <div class="bg-slate-900/40 border border-white/10 p-2.5 rounded-[2rem] shadow-2xl backdrop-blur-md">
                            <div class="group relative w-full rounded-3xl overflow-hidden cursor-pointer bg-slate-950" onclick="document.getElementById('image').click()">
                                ...
                                <img id="image-preview" src="{{ $series->image['webp'] }}" alt="{{ $series->name }}" class="w-full aspect-square object-cover transition-all duration-700 group-hover:scale-105 group-hover:opacity-60" />
                                ...
                                <input type="file" name="image" id="image" form="seriesForm" class="hidden" accept="image/*" onchange="previewImage(this)">
                            </div>
                        </div>
                        ...
                        <div class="bg-slate-900/40 border border-white/10 p-6 rounded-3xl shadow-xl backdrop-blur-md space-y-6 relative overflow-hidden">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative">
                                    <select id="rating" name="rating" form="seriesForm" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white font-medium appearance-none cursor-pointer focus:ring-2 focus:ring-emerald-500/50 outline-none transition-all">
                                        @foreach(['general' => '✅ General', 'sensitive' => '⚠️ Sensitive', 'questionable' => '🔞 R18 (Questionable)', 'explicit' => '🚫 Adult Explicit', 'unknown' => '❓ Uncategorized'] as $key => $label)
                                            <option value="{{ $key }}" {{ old('rating', $series->rating ?? 'unknown') == $key ? 'selected' : '' }}>{{ $label }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="pt-2">
                                <label class="relative flex items-center justify-between cursor-pointer group bg-slate-950/50 border border-slate-800 p-3 rounded-xl hover:border-slate-700 transition-colors">
                                    ...
                                    <div class="relative">
                                        <input type="hidden" name="debug" value="0" form="seriesForm">
                                        <input type="checkbox" name="debug" value="1" form="seriesForm" {{ old('debug', $series->debug) ? 'checked' : '' }} class="sr-only peer">
                ...
                <div class="lg:col-span-8 space-y-8">
                    <form id="seriesForm" action="{{ route('series.update', $series->id) }}" method="POST" enctype="multipart/form-data" @submit.prevent="submitForm" class="space-y-8">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="bg-slate-900/60 border border-white/10 p-6 sm:p-8 rounded-[2rem] shadow-xl backdrop-blur-md space-y-6 relative overflow-hidden group">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-2">
                                    <label for="name" class="block text-sm font-bold text-slate-400">Series Title</label>
                                    <div class="relative">
                                        <input type="text" id="name" name="name" value="{{ old('name', $series->name) }}" required class="w-full bg-slate-950 border border-slate-800 rounded-xl pl-4 pr-10 py-3 text-white font-bold text-lg focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all shadow-inner">
                                        <a href="https://www.google.com/search?q={{ urlencode($series->name) }}" target="_blank" title="Search Google" class="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 bg-slate-800 hover:bg-cyan-500/20 text-slate-400 hover:text-cyan-400 rounded-lg transition-all"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg></a>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-2" x-data="{ clearField() { let el = document.getElementById('slug'); el.value = ''; el.focus(); } }">
                                    ...
                                    <div class="relative">
                                        <input type="text" id="slug" name="slug" value="{{ old('slug', $series->slug) }}" required class="w-full bg-slate-950 border border-slate-800 rounded-xl pl-4 pr-10 py-3 text-cyan-200 font-mono text-sm focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all shadow-inner">
                            ...
                            <div class="space-y-2">
                                <label for="parents" class="block text-sm font-bold text-slate-400">Parent Series (Franchise)</label>
                                <div class="relative z-50">
                                    <select id="parents" name="parents[]" multiple placeholder="Search series..." class="w-full"></select>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-2" x-data="{ clearField() { let el = document.getElementById('description'); el.value = ''; el.focus(); } }">
                                    ...
                                    <textarea id="description" name="description" rows="4" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-white text-sm leading-relaxed resize-y focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all shadow-inner">{{ old('description', $series->description) }}</textarea>
                                </div>
                                ...
                                <div class="space-y-2" x-data="{ clearField() { let el = document.getElementById('keywords'); el.value = ''; el.focus(); } }">
                                    ...
                                    <textarea id="keywords" name="keywords" rows="4" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-white text-sm leading-relaxed resize-y focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all shadow-inner">{{ old('keywords', $series->keywords) }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div x-data="{ 
                            ...
                            async generateAndCopyPrompt() { 
                                ...
                                try { 
                                    const response = await fetch('{{ route('seo.series', $series->id) }}', { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } }); 
                            ...
                            applyJson() { 
                                ...
                                try { 
                                    ...
                                    const fields = [ 
                                        { id: 'seo_title', value: data.seo_title || data.title || data.meta_title }, 
                                        { id: 'seo_description', value: data.seo_description || data.description || data.meta_description }, 
                                        { id: 'seo_keywords', value: data.seo_keywords || data.keywords || data.meta_keywords }, 
                                        { id: 'keywords', value: data.alias || data.keywords || data.meta_keywords }, 
                                        { id: 'description', value: data.series_description || data.description } 
                                    ]; 
                                    let appliedCount = 0; 
                                    ...
                                    else alert('JSON fields are already filled or keys do not match. Clear fields manually to replace them.'); 
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/5 pb-4">
                                ...
                                <button type="button" @click="generateAndCopyPrompt" :disabled="isGenerating" class="px-5 py-2.5 bg-purple-500/10 text-purple-400 border border-purple-500/20 hover:bg-purple-500 hover:text-white rounded-xl text-xs font-bold transition-all shadow-sm flex items-center justify-center gap-2 disabled:opacity-50">
                                    ...
                                    <span x-text="isGenerating ? 'Analyzing...' : (copySuccess ? 'Copied to Clipboard!' : 'Generate SEO Prompt')"></span>
                                </button>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-2 md:col-span-2">
                                    <label for="seo_title" class="block text-sm font-bold text-slate-400">SEO Optimized Title</label>
                                    <div class="relative">
                                        <input type="text" id="seo_title" name="meta_title" value="{{ old('meta_title', $series->seo_title) }}" class="w-full bg-slate-950 border border-slate-800 rounded-xl pl-4 pr-10 py-3 text-white text-sm focus:ring-2 focus:ring-purple-500/50 outline-none transition-all shadow-inner">
                                ...
                                <div class="space-y-2">
                                    <div class="flex justify-between items-end">
                                        <label for="seo_description" class="block text-sm font-bold text-slate-400">SEO Meta Description</label>
                                    ...
                                    <textarea id="seo_description" name="meta_description" rows="3" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-white text-sm resize-none focus:ring-2 focus:ring-purple-500/50 outline-none transition-all shadow-inner">{{ old('meta_description', $series->seo_description) }}</textarea>
                                </div>
                                
                                <div class="space-y-2">
                                    <div class="flex justify-between items-end">
                                        <label for="seo_keywords" class="block text-sm font-bold text-slate-400">SEO Meta Keywords</label>
                                    ...
                                    <textarea id="seo_keywords" name="meta_keywords" rows="3" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-white text-sm resize-none focus:ring-2 focus:ring-purple-500/50 outline-none transition-all shadow-inner">{{ old('meta_keywords', $series->seo_keywords) }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-4 py-2">
                            ...
                            <span class="text-xs font-bold text-slate-500 uppercase tracking-widest text-center">Secondary Settings & Tags</span>
                            <div class="h-px bg-slate-800 flex-grow rounded-full"></div>
                        </div>

                        @php
                            $existingRelations = $series->children->map(function($child) {
                                return [
                                    'key' => uniqid(),
                                    'child_id' => (string) $child->id,
                                    'relation_type' => $child->pivot->relation_type ?? 'related'
                                ];
                            })->values();
                        @endphp
                        ...
                        <div x-data="{ open: false }" class="bg-slate-900/60 border border-white/10 rounded-3xl shadow-lg backdrop-blur-md relative overflow-hidden group">
                            ...
                            <div x-show="open" x-collapse x-cloak>
                                <div class="px-6 pb-6 pt-2 border-t border-white/5" x-data="{ relations: {{ json_encode($existingRelations) }}, addRelation() { this.relations.push({ key: Date.now(), child_id: '', relation_type: 'related' }); }, removeRelation(index) { this.relations.splice(index, 1); } }">
                                    ...
                                    <div class="space-y-3">
                                        <template x-for="(rel, index) in relations" :key="rel.key">
                                            <div class="flex flex-col sm:flex-row items-stretch bg-slate-950/80 rounded-xl border border-slate-800 overflow-hidden focus-within:border-pink-500/50 transition-all">
                                                <div class="relative bg-slate-900 border-b sm:border-b-0 sm:border-r border-slate-800 sm:w-40 flex-shrink-0">
                                                    <select :name="`relationships[${index}][relation_type]`" x-model="rel.relation_type" class="w-full h-full bg-transparent text-xs font-black text-pink-400 py-3 px-4 uppercase tracking-wider outline-none cursor-pointer appearance-none relative z-10 text-center sm:text-left">
                                                ...
                                                <select :name="`relationships[${index}][child_id]`" x-model="rel.child_id" class="flex-grow bg-transparent border-none text-sm text-white py-3 px-4 focus:outline-none focus:ring-0 appearance-none cursor-pointer">
                                                    <option value="" class="bg-slate-900">Select Series...</option>
                                                    @foreach(\App\Models\Series::all() as $item)
                                        ...
                                        <div x-show="relations.length === 0" class="text-center py-6 border border-dashed border-slate-700/50 rounded-xl bg-slate-950/30 text-xs text-slate-500">No series relationships established.</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div x-data="{ open: false }" class="bg-slate-900/60 border border-white/10 rounded-3xl shadow-lg backdrop-blur-md relative overflow-hidden group">
                            ...
                            <div x-show="open" x-collapse x-cloak>
                                <div class="px-6 pb-6 pt-2 border-t border-white/5" x-data="apiTagsManager({{ $series->apiTags->toJson() }})">
                                    <div class="flex justify-end mb-4">
        ...
        <div class="fixed bottom-0 left-0 right-0 z-50 bg-slate-950/80 backdrop-blur-xl border-t border-white/10 py-4 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
            <div class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4">
                <form action="{{ route('series.delete', $series->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to permanently delete this series? Associated characters will lose their relations, and this action cannot be undone.');" class="w-full sm:w-auto">
                    @csrf @method('DELETE')
    ...
    <script>
    const PRELOAD_PARENTS = @json($preloadParents ?? []);
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('apiTagsManager', (initialTags) => ({
            ...
            addTag() { 
                ...
                const seriesName = nameInput ? nameInput.value.trim() : '';
                const existingSources = this.tags.map(t => (t.source_api || '').toLowerCase());
                
                let targetSource = '';
                let targetValue = seriesName;

                if (!existingSources.includes('danbooru')) { targetSource = 'danbooru'; } 
                else if (!existingSources.includes('gelbooru')) { targetSource = 'gelbooru'; } 
                else if (!existingSources.includes('zerochan')) { targetSource = 'zerochan'; }

                if (targetSource === 'danbooru' || targetSource === 'gelbooru') { targetValue = seriesName.toLowerCase().replace(/\s+/g, '_'); } 
                else if (targetSource === 'zerochan') { targetValue = seriesName; }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if(parentsSelect) {
            new TomSelect('#parents', apiSelectConfig("{{ route('series.list') }}", { excludeId: SERIES_ID, preloadOptions: PRELOAD_PARENTS, preloadItems: PRELOAD_PARENTS.map(i => i.id) }));
        }
        ...
        if(nameInput) {
            nameInput.addEventListener('input', () => {
                ...
                if(headerName) headerName.textContent = val || 'Series';
                ...
                if(currentSlug === '' || currentSlug === expectedSlug) { slugInput.value = slugify(val); }
            });
        }
    });
    </script>
