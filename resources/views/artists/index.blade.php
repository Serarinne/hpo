...
<html lang="en-US" class="scroll-smooth">
  <head>
    ...
    <title>Artist Management - {{ env('APP_NAME') }}</title>
  ...
  <body class="bg-slate-950 text-slate-200 font-sans min-h-screen flex flex-col selection:bg-cyan-500 selection:text-white">
        ...
        <div class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="mb-8 flex flex-wrap items-end justify-between gap-6 border-b border-white/10 pb-6">
                <div class="space-y-2">
                    <div class="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-widest">
                        ...
                        <span class="text-slate-400">Artists</span>
                        <span class="text-slate-700">&bull;</span>
                    ...
                    <h1 class="text-3xl sm:text-5xl font-black text-white tracking-tight leading-none">
                        Artist <span class="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 drop-shadow-sm">Management</span>
                    </h1>
                    <p class="text-sm text-slate-400 font-medium pt-1">Browse, filter, and manage all your artist profiles.</p>
                </div>
            </div>
                ...
                <div class="relative z-10 flex flex-col gap-5 p-5 sm:p-6">
                    <div class="w-full relative z-10">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="search" id="search" value="{{ request('search') }}" placeholder="Search artist by name or ID..." class="w-full bg-slate-950 border border-slate-800 rounded-xl pl-10 pr-4 py-3 text-sm text-white placeholder-slate-600 focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all shadow-inner">
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col md:flex-row gap-4 items-end flex-wrap relative z-20">
                        ...
                        <div class="w-full md:w-44 relative">
                            ...
                            <details class="group custom-dropdown relative">
                                <summary class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all shadow-inner cursor-pointer list-none flex justify-between items-center select-none">
                                    ...
                                    <span class="text-sm truncate mr-2 {{ $wallpaperCount > 0 ? 'text-pink-400 font-bold' : 'text-slate-400 font-medium' }}">{{ $wallpaperCount > 0 ? $wallpaperCount . ' Selected' : 'Any Status' }}</span>
                                    <svg class="w-4 h-4 text-slate-500 group-open:rotate-180 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7" /></svg>
                                </summary>
                                <div class="absolute top-full left-0 md:right-0 md:left-auto z-50 w-full mt-2 bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl p-2 flex flex-col gap-1">
                        ...
                        <div class="w-full md:w-auto flex flex-1 gap-3 md:justify-end">
                            ...
                            @if(request()->anyFilled(['search', 'seo', 'workflow', 'has_wallpaper']))
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 mb-12 relative z-10">
                @forelse($artists as $artist)
                    ...
                    <a href="{{ route('artists.edit', ['id' => $artist->id]) }}" class="group block relative rounded-[1.5rem] overflow-hidden border border-white/5 bg-slate-900/60 shadow-lg hover:shadow-[0_0_20px_rgba(34,211,238,0.15)] hover:border-cyan-500/30 transition-all duration-300 backdrop-blur-sm transform hover:-translate-y-1">
                        <div class="aspect-square overflow-hidden bg-slate-950 relative {{ !$hasImage ? 'flex justify-center items-center p-6' : '' }}">
                            @if($hasImage)
                                <picture class="w-full h-full block">
                                    ...
                                    <img src="{{ $artist->image['jpg'] }}" alt="{{ $artist->name }}" loading="lazy" width="400" height="400" class="w-full h-full object-cover group-hover:scale-105 group-hover:opacity-80 transition-all duration-700 ease-out" />
                                </picture>
                            @else
                        ...
                        <div class="absolute top-3 left-3 flex flex-col gap-1.5 z-30">
                            <button type="button" x-data="{ isDebug: {{ $artist->debug ? 'true' : 'false' }} }" @click.prevent.stop="isDebug = !isDebug; toggleDebug({{ $artist->id }}, isDebug)" class="pointer-events-auto h-7 px-2 rounded-lg border backdrop-blur-md transition-all flex items-center justify-center shadow-lg font-bold" :class="isDebug ? 'bg-amber-500/20 text-amber-400 border-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.4)]' : 'bg-slate-950/60 text-slate-400 border-white/10 hover:border-white/20 hover:text-white hover:bg-slate-900'" title="Toggle Debug Mode">
                        ...
                        <div class="absolute inset-x-0 bottom-0 p-4">
                            <h3 class="text-white font-bold text-sm leading-snug line-clamp-2 group-hover:text-cyan-400 transition-colors">{{ $artist->name }}</h3>
                        </div>
                    </a>
                @empty
                    <div class="col-span-full w-full flex flex-col items-center justify-center py-20 px-4 text-center bg-slate-900/40 border border-dashed border-slate-700 rounded-[2rem] backdrop-blur-md shadow-inner">
                        ...
                        <h3 class="text-lg font-black text-white mb-2">No Artists Found</h3>
                        <p class="text-sm font-medium text-slate-500 max-w-sm mx-auto">
                            {{ request('search') ? "We couldn't find any artists matching your current filter criteria." : "There are no artists available yet." }}
                        </p>
                        @if(request()->anyFilled(['search', 'seo', 'workflow', 'has_wallpaper']))
    ...
    <script>
      ...
      window.toggleDebug = async function(artistId, isDebug) {
        try {
          const urlTemplate = "{{ route('artists.toggle-debug', ['id' => '__ID__']) }}";
          const url = urlTemplate.replace('__ID__', artistId);
          ...
          if (!data.success) {
            console.error('Failed to update debug status:', data.message);
          } else {
            console.error('Failed to update debug status:', data.message);
          }
        } catch (error) {
